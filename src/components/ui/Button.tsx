...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
    ...
    const renderIcon = () => {
        ...
        // Merge className if icon already has one
        if (React.isValidElement(icon)) {
            const existingClassName = (icon.props && (icon.props as React.SVGProps<SVGSVGElement>).className) || '';
            const mergedClassName = `${existingClassName} ${children ? (iconPosition === 'left' ? 'mr-2' : 'ml-2') : ''} h-5 w-5`.trim();
